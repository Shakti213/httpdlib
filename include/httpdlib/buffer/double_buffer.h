...
template <typename T>
auto make_double_buffer(T *adapter) -> std::unique_ptr<double_buffer<T>> {
    return std::unique_ptr<double_buffer<T>>(new double_buffer<T>(adapter));
}
template <typename T>
auto make_double_buffer(std::unique_ptr<T> adapter)
    -> std::unique_ptr<double_buffer<T>> {
    return std::unique_ptr<double_buffer<T>>(
        new double_buffer<T>(std::move(adapter)));
}
}
}
