...
public:
    ...
    void start_server(std::uint16_t port) {
        ...
        m_acceptor.async_accept(new_connection->socket(), [new_connection, me=this](auto ec) {
    ...
    void handle_new_connection() {
        ...
        auto next_connection = std::make_shared<ConnectionHandler>(m_io_service);
        m_acceptor.async_accept(next_connection->socket(), [next_connection, me=this](auto ec) {
...
public:
    ...
    void data_read(std::size_t bytes_read) {
        ...
        if(m_request.finished()) {
            ...
            m_response.reset(new httpdlib::memory_response(httpdlib::memory_response::default_for_code(m_request.parse_result())));
            write_data();
        }
        else {
            m_response.reset();
...
private:
    ...
    void read_data() {
        m_socket.async_read_some(boost::asio::buffer(m_recv_data), [me=shared_from_this()](auto ec, auto sz) {
            if(ec) {
                return;
            }
            me->data_read(sz);
    ...
    void write_data() {
        auto writer = [&s=m_socket, me=shared_from_this()](const char *p, std::size_t max_size) {
            s.async_write_some(boost::asio::buffer(p, max_size), [me2=me](auto ec, auto cnt) {
                if(ec) {
                    return;
                }
                me2->data_sent(cnt);
