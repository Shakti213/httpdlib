...
void response_generator::add_filter(std::unique_ptr<response_generator_filter> filter)
{
...
{
    return std::all_of(std::begin(m_filters), std::end(m_filters), [&request](const auto &filter_ptr) {
        return filter_ptr->passes_filter(request);
    });
}

bool response_generator::priv_is_handler_for_request(const request &request)
{
    return true;
}

bool httpdlib::interface::response_generator::is_handler_for_request(const request &request)
{
